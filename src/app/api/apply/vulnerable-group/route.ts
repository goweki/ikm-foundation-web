...
// interface ApplicationVulnaerableGroup {
//   email: string;
//   phone: string;
//   name: string;
...
export async function POST(req: NextRequest) {
  try {
    ...
    const post_response_ = await fetch(
      `https://api.airtable.com/v0/${airtableBaseID}/tblG8eqTOmCBmeGNr`,
      {
        method: "POST",
        headers: {
    ...
    if (!post_response_.ok) {
      ...
      console.warn(
        "FAILED: in routeHandler POST: /api/apply/vulnerable-group: ",
        errorMessage
      );
    ...
    console.error("ERROR in POST: /api/apply/vulnerable-group", error);

    return NextResponse.json(
      {
        message: "Error sending vulnerable-group application",
        error: error.message,
      },
      { status: 500 }
    );
