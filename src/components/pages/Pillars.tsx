      ...
      <div id="pillars" className="max-w-7xl mx-auto px-6">
        <div className="flex flex-wrap items-center justify-center mt-10 lg:mt-16 gap-4 lg:gap-8">
          {pillars.map((item, i) => (
            <article
              key={i}
              className="group relative aspect-video h-96 w-xl overflow-hidden rounded-xl shadow-md hover:shadow-2xl"
            >
              {/* Background image */}
              <Image
              ...
              {/* Gradient + content */}
              <div className="absolute inset-0 bg-linear-to-t from-black/0 to-transparent to-90% text-white transition-all duration-300 group-hover:bg-linear-to-t group-hover:from-black/60 group-hover:transition-all group-hover:duration-500">
                <h2 className="text-3xl lg:text-5xl absolute bottom-8 left-8 m-0 font-extrabold uppercase transition-all delay-300 duration-100 ease-out group-hover:bottom-1/2 group-hover:delay-0 group-hover:duration-300">
                  {item.title}
                </h2>
